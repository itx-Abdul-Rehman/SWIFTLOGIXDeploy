...
const RiderShipments = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch('http://localhost:3000/get-rider-shipments',
                {
                    method: 'POST',
                    credentials: 'include',
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://localhost:3000/rider-shipments',
                    { credentials: 'include' }
                );
                const result = await response.json();
                if (result.success) {
    ...
    const handleCancel = async (index) => {
        try {
            const response = await fetch("http://localhost:3000/rider-cancel-shipment",
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
