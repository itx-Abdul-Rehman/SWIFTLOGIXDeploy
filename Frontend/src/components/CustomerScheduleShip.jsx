...
const CustomerScheduleShip = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://13.203.194.4:3000/customer-scheduleship',
                    { credentials: 'include' }
                );
    ...
    const handleWithCardPayment = async () => {
        try {
            const response = await fetch('http://13.203.194.4:3000/checkout', {
                ...
                body: JSON.stringify({
                    senderFormData, receiverFormData, packageFormData, packagePrice,
                    endpoint: 'customer-scheduleship'
                })
            })
    ...
    const handleWithWalletPayment = async () => {
        try {
            const response = await fetch('http://13.203.194.4:3000/wallet-payment',
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
            ...
            setPaymentMethod(res.paymentmethod)
            packageFormData.paid = res.paid;
            packageFormData.payby = res.payby;
            if (res.success === true) {
                step.current++;
                step.current++;
                setActiveStep(6);
    ...
    const handleUseMyProfile = () => {
        setSenderFormData({
            ...
            sendercontact: userData.mobileno,
            senderemail: userData.email,
            sendercnic: userData.cnic
        });
    }
    ...
    return (
        <div className='flex '>
            ...
            {/* Right Content */}
            <div className={`flex-1 w-full min-[1226px]:ml-[18vw]  h-screen bg-white overflow-y-auto`}>
                <div className='w-full bg-[#009688] text-white flex justify-center items-center p-8'>
                    <div className='md:text-5xl text-4xl font-semibold'>Schedule a Ship</div>
                </div>
                {/* Process steps */}
                ...
                {currentStep === "payment" &&
                    <Payment
                        ...
                        onWalletPayment={handleWithWalletPayment}
...
export default CustomerScheduleShip;