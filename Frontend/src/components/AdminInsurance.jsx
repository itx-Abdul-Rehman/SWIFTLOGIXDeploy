...
const AdminInsurance = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/get-insurance-claims`,
                {
                    method: 'POST',
                    credentials: 'include',
    ...
    const handleAcceptandReject = async (index, claimStatus) => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/update-insurance-claims`,
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
